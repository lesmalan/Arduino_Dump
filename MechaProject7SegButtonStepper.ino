...
#include <Stepper.h> // Include stepper motor library to control tuning mechanism

#define STEPS 64 // Define the number of steps per revolution for the stepper motor
...
// Initialize stepper motor with the defined steps and control pins
...
void setup() {
  ...
  stepper.setSpeed(30);  // Set stepper motor speed (RPM)
...
void loop() {
  ...
  if (stringNote == 6) {
    ...
    if (adjustedFrequency < 78.28) { // If frequency is too low, tighten the string
      stepper.step(75);  // Move stepper motor forward
    }
    if (adjustedFrequency > 86.52) { // If frequency is too high, loosen the string
      stepper.step(-75); // Move stepper motor backward
    }
  }
  if (stringNote == 5) {
    adjustedFrequency = peakFrequency - 12; // A string correction
        // Simple tuning logic: move stepper motor based on frequency range
    if (adjustedFrequency < 104.5) { // If frequency is too low, tighten the string
      stepper.step(75);  // Move stepper motor forward
    }
    if (adjustedFrequency > 115.5) { // If frequency is too high, loosen the string
      stepper.step(-75); // Move stepper motor backward
    }
  }
  if (stringNote == 4) {
    adjustedFrequency = peakFrequency - 15; // D string correction
    if (adjustedFrequency < 139.4885) { // If frequency is too low, tighten the string
      stepper.step(75);  // Move stepper motor forward
    }
    if (adjustedFrequency > 154.1715) { // If frequency is too high, loosen the string
      stepper.step(-75); // Move stepper motor backward
    }
  }
  if (stringNote == 3) {
    adjustedFrequency = peakFrequency - 21.4; // G string correction
    if (adjustedFrequency < 186.2) { // If frequency is too low, tighten the string
      stepper.step(75);  // Move stepper motor forward
    }
    if (adjustedFrequency > 205.8) { // If frequency is too high, loosen the string
      stepper.step(-75); // Move stepper motor backward
    }
  }
  if (stringNote == 2) {
    adjustedFrequency = peakFrequency - 27; // B string correction
    if (adjustedFrequency < 235.593) { // If frequency is too low, tighten the string
      stepper.step(75);  // Move stepper motor forward
    }
    if (adjustedFrequency > 259.287) { // If frequency is too high, loosen the string
      stepper.step(-75); // Move stepper motor backward
    }
  }
  if (stringNote == 1) {
    adjustedFrequency = peakFrequency - 32; // High E string correction
    if (adjustedFrequency < 313.1485) { // If frequency is too low, tighten the string
      stepper.step(75);  // Move stepper motor forward
    }
    if (adjustedFrequency > 346.1115) { // If frequency is too high, loosen the string
      stepper.step(-75); // Move stepper motor backward
    }
  }
